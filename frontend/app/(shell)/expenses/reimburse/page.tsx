    ...
    <div className="p-4 sm:p-6 max-w-3xl mx-auto">
      ...
      {!loading && weeks.length > 0 && (
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="bg-red-500/6 border border-red-500/20 rounded-xl p-4">
            ...
            <p className="text-lg font-semibold font-mono text-red-300">{fmt(totalOutstanding)}</p>
          </div>
          <div className="bg-emerald-500/6 border border-emerald-500/20 rounded-xl p-4">
            ...
            <p className="text-lg font-semibold font-mono text-emerald-300">{fmt(totalPaid)}</p>
          </div>
          <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
            <p className="text-xs text-stone-500 mb-1">All time</p>
            <p className="text-lg font-semibold font-mono text-stone-200">{fmt(totalOutstanding + totalPaid)}</p>
          </div>
        </div>
      )}

      {/* Filter tabs */}
      <div className="flex gap-1 bg-stone-900 border border-stone-800 rounded-xl p-1 mb-6">
        {([
          { key: "all", label: "All" },
      ...
      {loading ? (
        ...
        <div className="border border-stone-800 rounded-xl p-12 text-center">
          ...
          {filter === "all" && (
            <p className="text-stone-600 text-sm">Receipts marked as reimbursable will appear here.</p>
          )}
        </div>
      ) : (
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      {week.outstanding > 0 && (
                        <p className="text-red-300 font-mono text-sm font-medium">{fmt(week.outstanding)} due</p>
                      )}
                      {week.paid > 0 && (
                        <p className="text-emerald-400/70 font-mono text-xs">{fmt(week.paid)} paid</p>
                      )}
                    </div>
                    <span className={`text-stone-500 text-sm transition-transform ${isExpanded ? "rotate-90" : ""}`}>→</span>
                  </div>
                </button>
                ...
                {isExpanded && (
                  <div className="border-t border-stone-800 bg-stone-950/50">
                        ...
                        <div className="space-y-1.5">
                          {week.reimbursements.map((r) => (
                            <div key={r.id} className="flex items-center justify-between">
                              <div>
                                <p className="text-stone-300 text-sm font-mono">{fmt(r.amount)}</p>
                                <p className="text-stone-500 text-xs">{fmtDate(r.paid_at)}{r.note ? ` · ${r.note}` : ""}</p>
                              </div>
                              <button
                    ...
                    <div className="px-4 py-3 border-b border-stone-800">
                      ...
                      {week.outstanding > 0 && (
                        <div className="flex justify-between text-sm pt-1 border-t border-stone-800 mt-1">
